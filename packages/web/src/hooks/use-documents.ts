...
export function useDocuments() {
	...
	const rows: DocumentRow[] = useMemo(() => docs, [docs]);
	...
	function handlePreviewSelected(document: DocumentRow) {
	...
	const prefetchDocument = (documentId: string) => {
		queryClient.prefetchQuery({
			...
			queryFn: () =>
				getDocumentRecord({ data: documentId }).then((doc) => {
					if (doc?.presignedUrl) {
						const img = new Image();
	...
	const goToPrev = () => {
		if (currentIndex > 0) {
			...
			if (currentIndex - 2 >= 0)
				prefetchDocument(rows[currentIndex - 2].documentId);
		}
	};
