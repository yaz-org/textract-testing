...
import { getDocumentRecord, processDocument } from "#/lib/server-fns";
...
export function PreviewDialog({
	...
	const reprocessMutation = useMutation({
		...
		onError: (caught) => {
			toast.error(
				caught instanceof Error ? caught.message : "Reprocessing failed.",
			);
		},
	});
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				...
				<div className="-mx-4 no-scrollbar max-h-[90dvh] overflow-y-auto px-4 flex flex-col gap-2">
					{isPending && <DocumentPreviewSkeleton />}
					...
					{doc && (
						<>
											...
											<div className="border-t border-slate-200 pt-6 mt-6 space-y-3">
												...
												{latestInference && (
													<p className="text-xs text-muted-foreground">
														Engine: {latestInference.inferenceType}
														{" · "}
														Confidence:{" "}
											...
											<div className="border-t border-slate-200 px-3 py-2 space-y-2">
												{inf.inferenceType === "doctr" && (
													<>
														<p>
															Pages:{" "}
															{(inf.raw as DocTRRawInference).pages ?? "—"}
														</p>
														<p>
															Lines:{" "}
															{(inf.raw as DocTRRawInference).lines ?? "—"}
														</p>
														<p>
															Confidence:{" "}
															{(
												...
												{inf.inferenceType === "onnxtr" && (
													<>
														...
														<p>
															Model:{" "}
															{
																(inf.raw as OnnxTRRawInference).modelInfo
																	.detArch
															}{" "}
															/{" "}
															{
							...
							{doc.textractResult && !doc.inferenceHistory && (
								<div className="border-t border-slate-200 pt-6 mt-6 space-y-6">
									{doc.textractResult.forms.length > 0 && (
										<section>
											...
											<div className="overflow-x-auto rounded-2xl border border-slate-200">
												<table className="min-w-full text-sm">
													...
													<TableBody>
														{doc.textractResult.forms.map((f) => (
															<TableRow key={f.key}>
									...
									{doc.textractResult.layout.length > 0 && (
										<section>
											<h3 className="mb-2 font-semibold">Layout</h3>
											<div className="space-y-1 text-sm">
												{doc.textractResult.layout.map((l) => (
													<div
